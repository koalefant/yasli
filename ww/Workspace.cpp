...
YASLI_CLASS(Space, SpaceEmpty, "Empty Space");
...
void Workspace::loadState(const char* fileName)
{
	...
	ia.setFilter(ww::SERIALIZE_STATE);
...
{
	if(vbox_){
		ASSERT(!vbox_->parent() || vbox_->parent()->refCount() >= 0);
		ASSERT(!vbox_->parent() || vbox_->parent()->refCount() < 0xFFFF);
		vbox_->_setParent(0);
		vbox_ = 0;
	}
...
void Space::setParent(Space* parent)
{
	...
	if(parent)
		if(ww::Widget* widget = parent->widget()){
...
// ---------------------------------------------------------------------------
YASLI_CLASS(Space, SpaceHBox, "SpaceHBox");
YASLI_CLASS(Space, SpaceVBox, "SpaceVBox");
...
void SpaceSplitterImpl::onMessageRButtonDown(UINT button, int x, int y)
{
	if(joining_){
		...
		if(splitterIndex >= 0){
			...
			menu.root().add(TRANSLATE("Split"), splitterIndex).connect(this, &Self::onMenuSplit);
			menu.root().add(TRANSLATE("Join"), splitterIndex).connect(this, &Self::onMenuJoin);
...
{
	PopupMenu menu(100);
	menu.root().add(TRANSLATE("Split")).connect(owner_, &SpaceHeader::onMenuSplit);
