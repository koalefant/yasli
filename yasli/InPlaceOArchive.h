...
public:
	...
	void inPlacePointer(void** pointer);
...
private:
	...
	template<class T>
	bool findOffset(size_t* offset, const T& member, const char* name) { return findOffset(offset, (void*)&member, sizeof(T), name); }
	...
	typedef std::map<void*, Chunk> AddressToChunkMap;
	AddressToChunkMap chunks_; // mapping from original addresses of all structures to ther locations inside image
	typedef std::map<void*, Chunk*> EndAddressToChunkMap;
	EndAddressToChunkMap chunkEnds_;

	typedef std::vector<size_t> PointerOffsets;
	PointerOffsets pointerOffsets_; // pointers actual for current chunk
	PointerOffsets allPointers_; // list of all pointers existing in image
